...
def uninstall(data_home: Path | None = None) -> int:
    """Remove the generated menu entry and icon. Returns an exit status.

    The counterpart to install(), and necessarily a manual step: an
    AppImage that has been deleted can no longer clean up after itself,
    so the entry it installed would otherwise linger in the launcher
    pointing at a path that no longer exists. Self-healing covers a
    MOVED AppImage (it runs again from the new path and rewrites Exec);
    nothing can cover a deleted one, which is why this has to be invoked
    before the file goes away.

    The marker check from install() applies in reverse: an entry we did
    not generate is one the user wrote deliberately, and refusing to
    delete it matters more here than during install -- getting it wrong
    destroys work instead of merely declining to update it. The icon is
    tied to that same decision rather than checked separately: this
    exact path is one only install() ever writes, so if the entry is
    ours the icon beside it is too, and if the entry is the user's they
    may well be relying on the icon it names.

    Returns 0 when the system ends up in the requested state (entry
    gone, including when it was never there), and 1 when removal was
    ...
    icons_root = home / "icons" / "hicolor"
    entry_path = applications_dir / f"{APP_ID}.desktop"
    icon_path = icons_root / ICON_SIZE / "apps" / f"{APP_ID}.png"

    if not _is_ours(entry_path):
        print(
            f"duriancalc: {entry_path} was not generated by DurianCalc, "
            f"leaving it and its icon alone. Remove it by hand if you meant to.",
            file=sys.stderr,
        )
        return 1
    ...
    for path in (entry_path, icon_path):
        try:
            ...
            failed = True
            print(f"duriancalc: could not remove {path} ({exc})", file=sys.stderr)

    if removed:
        # Same reasoning as in install(): only pay for a cache refresh
        # when something actually changed on disk.
        _refresh_caches(applications_dir, icons_root)
        for path in removed:
            print(f"duriancalc: removed {path}")
    else:
        print("duriancalc: no generated menu entry or icon found, nothing to remove")
